...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    return (IDandRatingsTuple[0], (len(IDandRatingsTuple[1]), 1.0 * sum(IDandRatingsTuple[1]) / len(IDandRatingsTuple[1])))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                             ...
                             .map(lambda rec: (rec[1][1][1], rec[1][0], rec[1][1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                       ...
                       .mapValues(lambda rec: math.pow(rec[0] - rec[1], 2))
...
print trainingRDD.map(lambda rec: rec[2]).mean()
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                      .map(lambda rec: (rec[1][0][0], rec[1][1], rec[1][0][1]))
                      .filter(lambda rec: rec[2] > 75))
