...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda tokens: len(tokens[1])).reduce(lambda a, b: a + b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, key=lambda rec: -len(rec[1]))
...
def tf(tokens):
    ...
    for token in set(tokens):
        tf[token] = 1.0 * tokens.count(token) / len(tokens)
    return tf
...
def idfs(corpus):
    ...
    return (tokenSumPairTuple.mapValues(lambda n: 1.0 * N/n))
...
def dotprod(a, b):
    ...
    """
    return sum([a[token] * b[token] for (token, count) in a.iteritems() if a.get(token) and b.get(token)])
...
avgSimDups = trueDupsRDD.map(lambda record: record[1][0]).reduce(lambda a, b: a + b) / trueDupsCount

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda record: record[1][1] == None))
avgSimNon = nonDupsRDD.map(lambda record: record[1][0]).reduce(lambda a, b: a + b) / nonDupsRDD.count()
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda rec: (rec[0], tfidf(rec[1], idfsFullWeights)))
googleWeightsRDD = googleFullRecToToken.map(lambda rec: (rec[0], tfidf(rec[1], idfsFullWeights)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda rec: (rec[0], norm(rec[1])))
amazonNormsBroadcast = sc.broadcast(amazonNorms.collectAsMap())
googleNorms = googleWeightsRDD.map(lambda rec: (rec[0], norm(rec[1])))
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum([amazonWeightsBroadcast.value.get(amazonRec).get(token)
             * googleWeightsBroadcast.value.get(googleRec).get(token)
             for token in tokens])
